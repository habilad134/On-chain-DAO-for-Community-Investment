...
(define-public (create-investment
    ...
    (begin
        ...
        (asserts! (<= allocated-amount (var-get total-treasury))
...
(define-public (update-investment-value
        (investment-id uint)
        (new-value uint)
    )
    (let ((investment (unwrap! (get-investment investment-id) ERR-INVESTMENT-NOT-FOUND)))
        (asserts!
            (or
                (is-eq tx-sender (get manager investment))
        ...
        (let (
                (allocated (get allocated-amount investment))
...
(define-public (close-investment (investment-id uint))
    (let ((investment (unwrap! (get-investment investment-id) ERR-INVESTMENT-NOT-FOUND)))
        ...
        (let (
                (current-value (get current-value investment))
