...
(define-map qf-votes { id: uint, voter: principal } { choice: uint })
...
(define-public (qf-cast-vote (id uint) (choice uint))
  (let
    (
      (prop (unwrap! (map-get? qf-proposals id) (err ERR_QF_NOT_FOUND)))
    )
    (begin
      (asserts! (not (get finalized prop)) (err ERR_QF_FINALIZED))
      (asserts! (not (> block-height (get end-height prop))) (err ERR_QF_VOTING_CLOSED))
      (asserts! (or (is-eq choice QF_CHOICE_FOR) (or (is-eq choice QF_CHOICE_AGAINST) (is-eq choice QF_CHOICE_ABSTAIN))) (err ERR_QF_BAD_CHOICE))
      (match (map-get? qf-votes { id: id, voter: tx-sender })
        ...
        (let
          (
            ...
            (a (get against prop))
            (ab (get abstain prop))
            (new-for (if (is-eq choice QF_CHOICE_FOR) (+ f u1) f))
            (new-against (if (is-eq choice QF_CHOICE_AGAINST) (+ a u1) a))
            (new-abstain (if (is-eq choice QF_CHOICE_ABSTAIN) (+ ab u1) ab))
          )
          (begin
            (map-set qf-votes { id: id, voter: tx-sender } { choice: choice })
            (map-set qf-proposals id
              {
...
(define-public (qf-finalize (id uint))
  (let
    ...
    (begin
      (asserts! (not (get finalized prop)) (err ERR_QF_FINALIZED))
      (asserts! (> block-height (get end-height prop)) (err ERR_QF_VOTING_CLOSED))
      (let
        (
        ...
        (begin
          ...
          (let
            (
              (passed? (> (get for prop) (get against prop)))
            )
            (begin
              (map-set qf-proposals id
                {
...
(define-read-only (qf-get-vote (id uint) (voter principal))
  (map-get? qf-votes { id: id, voter: voter })
)
